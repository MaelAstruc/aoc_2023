...
pub(crate) fn part_1(input: &str) -> u32 {
    ...
    let mut positions: Vec<usize> = vec![1, n_cols];
    ...
    let min_up = min_vector(&last_tile.cumulated_up);
    let min_down = min_vector(&last_tile.cumulated_down);
    let min_left = min_vector(&last_tile.cumulated_left);
    let min_right = min_vector(&last_tile.cumulated_right);
...
struct Tile {
    value: u32,
    cumulated_up: [Option<u32>; 3],
    cumulated_down: [Option<u32>; 3],
    cumulated_left: [Option<u32>; 3],
    cumulated_right: [Option<u32>; 3],
}
...
impl Tile {
    fn new(value: u32) -> Self {
        Self {
            value,
            cumulated_up: [None; 3],
            cumulated_down: [None; 3],
            cumulated_left: [None; 3],
            cumulated_right: [None; 3],
        }
    }

    fn update(&mut self, from: Direction, tile: &Tile) {
        let min_up = min_vector(&tile.cumulated_up);
        let min_down = min_vector(&tile.cumulated_down);
        let min_left = min_vector(&tile.cumulated_left);
        let min_right = min_vector(&tile.cumulated_right);
